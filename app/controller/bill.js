...
class BillController extends Controller{
  ...
  async list(){
    ...
    // const yeardate = moment(Number(_date)).format('YYYY')
    ...
    try{
      ...
      /* const yearlist = list.filter(item =>{
        if(type_id != 'all'){
          return moment(Number(item.date)).format('YYYY') == yeardate && type_id == item.type_id
        }
        return moment(Number(item.date)).format('YYYY') == yeardate
      }) */ 
  ...
  async yearlist(){
    ...
    try{
      ...
      // 筛选年度账单
      const yearlist = list.filter(item =>{ 
          return moment(Number(item.date)).format('YYYY') == date  
      }) 
      ...
      yearlist.map((item)=>{
        item.date = moment(Number(item.date)).format('YYYY-MM-DD')
      })

      let totalExpense = yearlist.reduce((curr , item)=>{
        if(item.pay_type == 1){
          curr+=Number(item.amount)
          return curr
        }
      ...
      let totalIncome = yearlist.reduce((curr,item)=>{
        if(item.pay_type == 2){
          curr += Number(item.amount)
          return curr
        }
  ...
  async detail(){
    ...
    try{
      ...
      ctx.body={
        ...
        msg:'请求成功',
        // data: detail 原版
        data: detail == null ? '无数据' : detail // 改的
      }
    } catch(error){
        ctx.body={
          code:500,
